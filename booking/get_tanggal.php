...
$sql = mysqli_query($koneksidb, "SELECT * from jadwal where status ='0' and availability ='0' order by jam asc");
...
// end validasi
if ($validasi != 'YES') {
	$html .= "Tanggal belum dipilih."; // Tambahkan tag option ke variabel $htm
} else {
	while ($data = mysqli_fetch_array($sql)) { // Ambil semua data dari hasil eksekusi $sql
		$html .= "<option value='" . $data['jam'] . "'>" . $data['jam'] . "    </option>"; // Tambahkan tag option ke variabel $html
	}
}
...
$callback = array('datajam' => $html); // Masukan variabel html tadi ke dalam array $callback dengan index array : data_kota
