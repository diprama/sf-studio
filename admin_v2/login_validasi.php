...
if (isset($_POST['btnLogin'])) {
	...
	if (count($pesanError) >= 1) {
		...
		function acak53(int $panjang): string
		{
			$karakter = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz123456789';
			$string = '';
			for ($i = 0; $i < $panjang; $i++) {
				$pos = random_int(0, strlen($karakter) - 1);
				$string .= $karakter[$pos];
			}
			return $string;
		}
		...
		if (mysqli_num_rows($myQry) >= 1) {
			...
			if ($myData['user_group'] == 'USER') {
				echo "<meta http-equiv='refresh' content='0; url=?page=Main-User'>";
			} else {
